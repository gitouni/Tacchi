...
class Deformation_Pyramid ():
    ...
    def reset(self):
        for i in range(self.n_hierarchy):
            self.pyramid[i]._reset_parameters()
...
class NDPLayer(nn.Module):
    ...
    def forward(self, x) -> Tuple[Union[None, torch.Tensor], torch.Tensor]:

        feat = self.posenc(x)
        feat = self.input(feat)
        feat = self.mlp(feat)

        t = self.mlp_scale * self.trn_branch ( feat )

        if self.motion == "SE3":
            R = self.get_Rotation(feat)
            x_ = (R @ x[..., None]).squeeze() + t

        elif self.motion == "Sim3":
            R = self.get_Rotation(feat)
            s = self.mlp_scale * self.s_branch(feat) + 1  # optimization starts with scale==1
        ...
        if self.nonrigidity_est:
            nonrigidity = self.sigmoid( self.mlp_scale * self.nr_branch(feat) )
    ...
    def get_Rotation (self, feat):

        R = self.mlp_scale * self.rot_brach( feat )
...
class Nerfies_Deformation(nn.Module):
    ...
    def warp (self, x, iter) :
        feat = self.posenc(x, iter)
        feat = self.input(feat)
        feat = self.mlp(feat)
        w = self.w_branch(feat)
        v = self.v_branch(feat)
